...
describe('AuthService', () => {
  ...
  it('throws an error if user signs up with email that is in use', async () => {
    ...
    await expect(
      service.signup('test@test.com', 'pass'),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it('throws if signin is called with an unused email', async () => {
    await expect(
      service.signin('test@test.com', 'pass'),
  ...
  it('throws if an invalid password is provided', async () => {
    ...
    await expect(service.signin('test@test.com', '123')).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('returns a user if correct credentials are provided', async () => {
    ...
    const user = await service.signin('user@test.com', 'password');
